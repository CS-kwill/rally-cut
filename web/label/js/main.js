...
let reviewCands = null;  // 리뷰 모드: 특정 클래스만 재검토(명시적 후보 목록, 슬라이더 무시)
...
function rebuildCandidates() {
  if (reviewCands) {       // 리뷰 모드: 명시적 후보만(슬라이더 무시, 접기 없음)
    cands = reviewCands.map((c, i) => ({
      id: String(i + 1).padStart(4, '0'), t: c.t, key: c.t.toFixed(2),
      pred: c.pred, conf: c.conf, ns: null, collapsed: false,
    }));
    renderList(); summarize(); return;
  }
...
function summarize() {
  if (reviewCands) {
    ...
    const cs = CLASSES.map((c) => counts[c.key] ? `${c.ko}${counts[c.key]}` : null).filter(Boolean).join(' ');
    $('summary').textContent = `리뷰 ${cands.length}개${cs ? ' · ' + cs : ''}`;
    return;
  }
...
function applySwingData(data) {
  // 리뷰 모드: 특정 클래스만 재검토 (명시적 cands 목록, 슬라이더 없음)
  if (data && Array.isArray(data.cands)) {
    reviewCands = data.cands.map((c) => ({ t: Number(c.t), pred: c.pred || null, conf: (c.conf != null ? c.conf : null) }));
    dataVideo = data.video || '';
    labels = {}; predsMap = {};
    reviewCands.forEach((c) => { const k = c.t.toFixed(2); predsMap[k] = { pred: c.pred, conf: c.conf, ns: 0 }; if (c.pred) labels[k] = c.pred; });
    $('tuner').hidden = true; $('exportBar').hidden = false;
    rebuildCandidates();
    status(`리뷰 모드 ${reviewCands.length}개 (${dataVideo || '?'}) — 맞으면 그대로, 틀리면 수정.`
      + (videoFile ? '' : ' ⚠ 같은 영상도 선택하세요'));
    return;
  }
  reviewCands = null;
